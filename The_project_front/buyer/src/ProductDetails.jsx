...
export default function ProductDetails() {
  ...
  const mainImage = current.image || null;
  ...
  const handleAddToCart = () => {
    ...
    if (quantity > (current.stock || 0)) {
      alert(`Only ${current.stock} available in stock`);
      return;
    }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.infoSection}>
          ...
          <div style={styles.price}>EGP {current.price}</div>
          <div style={{marginTop: '12px', color: current.stock > 0 ? '#27ae60' : '#e74c3c', fontWeight: '600'}}>
            {current.stock > 0 ? `${current.stock} in Stock` : 'Out of Stock'}
          </div>
          <div style={{marginTop: '8px', color: '#7f8c8d', fontSize: '14px'}}>
            Seller: {current.seller_email || current.seller_username || 'Unknown'}
          </div>
