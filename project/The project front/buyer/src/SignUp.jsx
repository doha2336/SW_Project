...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e) => {
    ...
    const strongEnough = pw.length >= 8 && /[0-9]/.test(pw) && /[A-Za-z]/.test(pw);
    if (!strongEnough) {
    ...
    try {
      ...
      // user will be auto-logged-in by register
      ...
      console.log('Registration successful, user type:', u.user_type);
      setTimeout(() => {
        if (u && u.user_type === 'buyer') {
          navigate('/buyer', { replace: true });
        } else if (u && u.user_type === 'seller') {
      ...
      if (data) {
        if (typeof data === 'string') msg = data;
          ...
          msg = Array.isArray(first) ? first.join('\n') : String(first);
        }
      }
