...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Ensure username uniqueness just in case (append suffix)
        original = username
        counter = 1
...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        # If frontend sends 'email', map it to 'username'
        if "email" in attrs and not attrs.get(self.username_field):
            try:
                user = User.objects.get(email=attrs["email"])
                attrs[self.username_field] = user.get_username()
            except User.DoesNotExist:
                pass  # Let base class handle invalid credentials or the view handle specific messaging
